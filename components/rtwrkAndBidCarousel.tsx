...
const RtwrkAndBidCarousel = ({
  ...
  const { data: rtwrkTimestampData } = useCall({
  ...
  const { execute: settleAuction } = useExecute({
    calls: {
      ...
      <div className={styles.previousRtwrksSide}>
        ...
        {isAuctionPage && (
          <Auction
            ...
            lastDrawingToSettle={!isCurrentDrawing && round > mintedCount}
          />
        )}
